        ...
        public static unsafe void CreateDirectory(string fullPath, byte[]? securityDescriptor = null)
        {
            ...
            fixed (byte* pSecurityDescriptor = securityDescriptor)
            {
                ...
                while (stackDir.Count > 0)
                {
                    ...
                    r = Interop.CreateDirectoryFromApp(name, secAttrs);
                    if (!r && (firstError == 0))
                    {
                        WIN32_ERROR currentError = (WIN32_ERROR)Marshal.GetLastPInvokeError();
                        ...
                        if (currentError != WIN32_ERROR.ERROR_ALREADY_EXISTS)
                        {
                        ...
                        else
                        {
                            ...
                            if (FileExists(name) || (!DirectoryExists(name, out currentError) && currentError == WIN32_ERROR.ERROR_ACCESS_DENIED))
                            {
                                firstError = currentError;
                                errorString = name;
                            }
            ...
            if ((count == 0) && !somepathexists)
            {
                ...
                {
                    throw Win32Marshal.GetExceptionForWin32Error(WIN32_ERROR.ERROR_PATH_NOT_FOUND, root);
                }
